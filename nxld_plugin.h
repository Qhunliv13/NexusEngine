...
typedef struct {
    ...
    int32_t min_param_count;                  /**< 最小参数数量 / Minimum parameter count / Mindestparameteranzahl */
    int32_t max_param_count;                  /**< 最大参数数量（-1表示无限制） / Maximum parameter count (-1 for unlimited) / Maximalparameteranzahl (-1 für unbegrenzt) */
 ...
 */
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_name_func)(char* name, size_t name_size);
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_version_func)(char* version, size_t version_size);
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_interface_count_func)(size_t* count);
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_interface_info_func)(size_t index, char* name, size_t name_size, 
                                                     char* description, size_t desc_size, 
                                                     char* version, size_t version_size);
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_interface_param_count_func)(size_t index,
                                                           nxld_param_count_type_t* count_type,
                                                           int32_t* min_count, int32_t* max_count);
typedef int32_t (NXLD_PLUGIN_CALL *nxld_plugin_get_interface_param_info_func)(size_t index, int32_t param_index,
